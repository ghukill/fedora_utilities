...
def indexItemsinSolr(item_ID):
	...
	os.system(Template("bash /var/opt/solr_utilities/sindex_item_153.sh $item_ID").substitute(item_ID=item_ID))
	print "item indexed."
...
def moveToProcessed():
	...
	try:
		os.system("mv /var/www/data/* /var/www/processed/")
	except:
		print "could not move files out of /data"
...
if processType == "full":
	...
	postProcessIngest()
	moveToProcessed()
	print "All items processed and ingested or both, great success!"
if processType == "process":
	item_ID_list = directoryRename() #REQUIRED
	permissionsChange(item_ID_list)
	processItems(item_ID_list)
	moveToProcessed()
	print "All items processed, great success!"
if processType == "ingest":	
	postProcessIngest()
	moveToProcessed()
